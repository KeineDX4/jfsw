...
// the Qt shell (belle_main.cpp) for the JFSW -> Symbian Belle port.
//
// Threading model:
...
//
// Logging: ONE file, sw.log, written by the engine's own buildsetlogfile("sw.log")
// into the WORKING DIRECTORY -- which the game thread chdir()s to BELLE_GAME_DIR
// (the game-data folder from belle_config.h, next to SW.GRP). No port diagnostics
// are mixed in and no debug output channel exists at all.
