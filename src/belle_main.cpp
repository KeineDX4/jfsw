// belle_main.cpp -- Qt 4.7.4 shell for the JFSW -> Symbian Belle port.
//
// The game's busy-loop (app_main -> Control -> InitGame -> Set_GameMode ->
// LogoLevel -> MenuLevel) runs on a QThread; the Qt main thread owns the
...
public:
    GameWorker()
    {
        ...
        // white screen with NO thread markers, NO heartbeat, NO sw.log --
...
protected:
    void run()
    {
        // v-final: no thread-lifecycle markers (only the engine's own sw.log is
        // written).
        try {
            char const *argv[2] = { "JFSW", NULL };
            app_main(1, argv);
        } catch (...) {
...
{
    // v-final: no sw.start marker (only the engine's own sw.log is written).
    // Ensure the game folder (BELLE_GAME_DIR from jfsw.pro, next to SW.GRP)
    // exists so the engine can open sw.log even before it reaches its own mkdir
    // in game.c. mkdir() creates only one level, so the parent drive root is
    // made first.
    ...
    // v-final: no PROCESS EXITING marker (only the engine's own sw.log exists).
    return rc;
}
