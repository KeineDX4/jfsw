// belle_layer.c -- real Qt baselayer for the JFSW -> Symbian Belle port.
//
...
// v26: real key names for the rebind window. The game's key-setup menu draws
// the current binding via getkeyname(); the old stub returned "" so every line
// showed "  -" and the user couldn't tell what was assigned. Delegate to the
// game's own scancode->name table (jfmact/keyboard.c) for consistent names.
// (kb_scancode is a uint8; declared by hand so we don't pull in keyboard.h,
// which needs jfmact/types.h included first.)
extern char *KB_ScanCodeToString(unsigned char scancode);
const char *getkeyname(int num) { return KB_ScanCodeToString((unsigned char)num); }
...
// engine's own sw.log is the only log channel.
...
// the engine's sw.log channel (buildprintf -> buildputs -> the logfile) instead
